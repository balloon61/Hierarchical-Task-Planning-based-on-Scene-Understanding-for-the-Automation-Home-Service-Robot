...
class ROBOT:
    ...
    def inverse(self, left_q, right_q, direction, offset):
        ...
        while ((init_pos - cur_pos) ** 2).sum() ** 0.5 <= offset:
            ...
            self.pub_left_arm_j1.publish(left_q[1] + self.l_offset1)
            self.pub_left_arm_j2.publish(left_q[2] + self.l_offset2)
            self.pub_left_arm_j3.publish(left_q[3] + self.l_offset3)

            self.pub_right_arm_j1.publish(right_q[1] + self.r_offset1)
            self.pub_right_arm_j2.publish(right_q[2] + self.r_offset2)
            self.pub_right_arm_j3.publish(right_q[3] + self.r_offset3)
