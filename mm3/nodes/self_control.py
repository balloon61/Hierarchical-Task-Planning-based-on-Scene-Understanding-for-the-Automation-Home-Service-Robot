...
def control_law(x_goal, y_goal, t_goal, px, py, theta, v_d, theta_d):
    if (x_goal - px) * (x_goal - px) + (y_goal - py) * (y_goal - py) < 0.05:
        ...
        return min(v_d * np.cos(theta_d - theta) + k1 * (np.cos(theta) * (x_goal - px) + np.sin(theta) * (y_goal - py)),
                   0.5), \
...
if __name__ == "__main__":
    ...
    for t in range(110):
        ...
        if t == 10:
            x_rec = position[-1][0]
            seconds_rec = rospy.get_time()
        twist.linear.x = 0.35 * Low_level_gain
        twist.linear.y = 0.0
        twist.linear.z = 0.0
    ...
    print("x:", x, x_rec)
    print("data:", x - x_rec)
    print("sec:", seconds_rec, rospy.get_time())
    y = position[-1][1]
    th = position[-1][2]
